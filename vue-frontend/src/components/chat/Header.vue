                ...
                <div class="tyn-appbar-content">
                    <ul class="tyn-appbar-nav tyn-appbar-nav-start">
                        <li class="tyn-appbar-item">
                            <router-link to="/index" class="tyn-appbar-link">
                    ...
                    <ul class="tyn-appbar-nav tyn-appbar-nav-end">
                        ...
                        <li class="tyn-appbar-item">
                            <a ref="notificationDropdown"  class="tyn-appbar-link dropdown-toggle position-relative" data-bs-toggle="dropdown" href="#" data-bs-offset="0,10" data-bs-auto-close="outside">
                            ...
                            <!-- <span class="tyn-appbar-badge position-absolute">{{ notificationCount }}</span> -->
                                            ...
                                            <div class="tyn-media-group align-items-start">
                                                ...
                                                <div class="tyn-media-col">
                                                    <div class="tyn-media-row">
                                                        <span v-if="notification.type=='send-request-friend'" class="message"><strong>{{notification.sender.name}}</strong > Added You</span>
                                                        <span v-if="notification.type=='send-message'" class="message"><strong>{{notification.sender.name}}</strong >Sent message</span>
                                                        <span v-if="notification.type=='friendship'" class="message">You and <strong>{{notification.sender.name}}</strong > are friends</span>
                                                        <span v-if="notification.type=='accept-request-friend'" class="message"><strong>{{notification.sender.name}}</strong > accept your friend request</span>
                                                    </div>
                                                    <div class="tyn-media-row has-dot-sap">
                                                        <span class="meta">{{ formatTimeAgo(notification.created_at)  }}</span>
                                                    </div>
                                                    <div v-if="notification.type=='send-request-friend'" class="tyn-media-row">
                                                        <ul class="tyn-btn-inline gap gap-3 pt-1">
                                                            <li>
                                                                <button @click="acceptFriendRequest(notification.sender_id,'accepted',notification.id)" class="btn btn-md btn-primary">
                                                            ...
                                                            <li>
                                                                <button @click="acceptFriendRequest(notification.sender_id,'reject',notification.id)" class="btn btn-md btn-light">
...
export default {
  ...
  setup() {
    ...
    onMounted(() => {
        ...
        Echo.private(`friend-request.${authStore.user.id}`)
            .listen('.friend.request.sent', (event) => {
                console.log("Received event: ", event);
                fetchNotification().catch(error => console.error("Lỗi fetchNotification:", error));
            });
        Echo.private(`friend-request-updated.${authStore.user.id}`)
            .listen('.friend.request.updated', async (data) => {
    ...
    const acceptFriendRequest = async (receiver_id,action,notification_id) => {
        try {
            await api.post(`/api/update-friend-request`, { receiver_id: receiver_id,action:action,notification_id:notification_id });
        } catch (error) {
    ...
    const  formatTimeAgo = (timestamp)=> {
        ...
        const diff = Math.floor((now - time) / 1000); // Chênh lệch tính bằng giây
    ...
    const fetchNotification = async () => {
        try {
            const response = await api.get('/api/fetch-notification');
